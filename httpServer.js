...
connector.prototype.createHttpServer = function() {
	...
	this.server = http.createServer(function(req, res) {
		try {
			var now = new Date();
			console.log("connect ok: --------start-------- " + now);
			var url = req.url;
		...
		parse_string(data, function(err, result) {
			for (var v in result) {
				if (v == "callbackReq") {
					...
					var xml_pay_message_res = json2xml(pay_message_res, {
						header: true
					});
					res.writeHead(200, {
		...
		parse_string(data, function(err, result) {
			for (var v in result) {
				if (v == "checkOrderIdReq") {
					...
					var json_pay_message_res = {
						"paymessages": pay_message_res
					};
					var xml_pay_message_res = json2xml(json_pay_message_res, {
						header: true
					});
					res.writeHead(200, {
						'Content-Type': 'application/text/xml; charset=utf-8'
